...
All work in one PR: code + tests + changelog stamp + version bump.
After merge: `scripts/release.py X.Y.Z` validates consistency, checks
CI on HEAD, then tags. No separate release PR.

CHANGELOG.md uses [Keep a Changelog](https://keepachangelog.com/en/1.1.0/)
format. Update it in the same PR as the code change — don't defer to a
separate release PR. Don't forget the comparison link at the bottom.
